...
async function run() {
  ...
  try {
    console.log('Querying verifications table schema...');
    const columns = await client.execute("PRAGMA table_info(verifications);");
    console.log('verifications columns:', columns.rows);
  } catch (err) {
    console.error(err);
  } finally {
    client.close();
