...
// Animation variants outside component — stable references, no recreation per render
...
// Skeleton placeholder for metric cards while data loads
...
function DashboardPage() {
  ...
  const stats = useMemo(() => {
    ...
    const totalPending = bills?.filter((b: any) => b.status === 'pending').reduce((s: number, b: any) => s + b.totalAmount, 0) ?? 0
    const totalOverdue = bills?.filter((b: any) => b.status === 'overdue').reduce((s: number, b: any) => s + b.totalAmount, 0) ?? 0
    ...
    return { totalUnits, occupiedUnits, availableUnits, maintenanceUnits, occupancyRate, totalIncome, totalExpenses, netProfit, totalPending, totalOverdue, pendingCount, overdueCount }
  }, [properties, units, payments, expenses, bills])

  const recentBills = useMemo(() => bills?.slice(0, 5) || [], [bills])

  const { netProfit, totalIncome, totalExpenses, totalOverdue, overdueCount, totalPending, pendingCount, occupancyRate, occupiedUnits, totalUnits, availableUnits, maintenanceUnits } = stats

  // Per-section loading flags — show skeleton instead of full-page spinner
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Metrics Grid — shows skeleton per card, not full-page spinner */}
      {isMetricsLoading ? (
