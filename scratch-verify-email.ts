...
async function run() {
  ...
  try {
    console.log('Updating email_verified to 1 for taufiqrusdhi.ez@gmail.com...');
    const res = await client.execute({
      sql: 'UPDATE users SET email_verified = 1 WHERE email = ?;',
      args: ['taufiqrusdhi.ez@gmail.com']
    });
    console.log('Update result:', res);

    const check = await client.execute("SELECT email, email_verified FROM users;");
